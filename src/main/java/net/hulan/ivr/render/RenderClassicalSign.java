...
public class RenderClassicalSign<T extends BlockClassicalSign.TileEntityClassicalSign> extends BlockEntityRendererMapper<T> implements IBlock, IGui, IDrawing {
    ...
    private static void renderCustomText(String signText, StoredMatrixTransformations storedMatrixTransformations, Direction facing, float size, float start, boolean flipCustomText, float maxWidth, int backgroundColor) {
        final KSDClientCache.DynamicResource dynamicResource = KSDClientData.DATA_CACHE.getSignText(signText, flipCustomText ? HorizontalAlignment.RIGHT : HorizontalAlignment.LEFT, (1 - BlockClassicalSign.SMALL_SIGN_PERCENTAGE) / 2, backgroundColor, ARGB_WHITE);
