...
public class RenderClassicalSign1Odd<T extends BlockClassicalSign.TileEntityClassicalSign1Odd> extends BlockEntityRendererMapper<T> implements IBlock, IGui, IDrawing {
    ...
    private void renderSide(T entity, MatrixStack matrices, VertexConsumerProvider vertexConsumers) {
    ...
    private void renderOppositeSide(T entity, MatrixStack matrices, VertexConsumerProvider vertexConsumers) {
        final BlockView world = entity.getWorld();
        if (world == null) {
            return;
        }
    ...
    public static void drawSign(MatrixStack matrices,
                                ...
                                Direction facing,
                                int backgroundColor,
                                RenderClassicalSign1Odd.DrawTexture drawTexture) {
        ...
        final boolean isExit = signId.equals(BlockClassicalSign.SignType.EXIT_LETTER.toString()) || signId.equals(BlockClassicalSign.SignType.EXIT_LETTER_FLIPPED.toString());
            ...
            if (platformPositions != null) {
                ...
                for (int i = 0; i < selectedIdsSorted.size(); i++) {
                    ...
                    RenderTrains.scheduleRender(IVRClientData.DATA_CACHE.getDirectionArrow(selectedIdsSorted.get(i),
                                    ...
                                    false,
                                    margin / size,
                                    (right - left) / (bottomOffset - topOffset),
                                    backgroundColor,
                                    ARGB_WHITE,
                                    backgroundColor).resourceLocation,
                            true,
    ...
    private static void renderCustomText(String signText, StoredMatrixTransformations storedMatrixTransformations, Direction facing, float size, float start, boolean flipCustomText, float maxWidth, int backgroundColor) {
        final IVRClientCache.DynamicResource dynamicResource = IVRClientData.DATA_CACHE.getSignText(signText, flipCustomText ? HorizontalAlignment.RIGHT : HorizontalAlignment.LEFT, (1 - BlockClassicalSign.SMALL_SIGN_PERCENTAGE) / 2, backgroundColor, ARGB_WHITE);
    ...
    public static CustomResources.CustomSign getSign(String signId) {
        try {
            final BlockClassicalSign.SignType sign = BlockClassicalSign.SignType.valueOf(signId);
