...
public class ModernSignScreen extends ScreenMapper implements IGui {
    ...
    private final boolean isModernSign;
    ...
    public ModernSignScreen(BlockPos signPos) {
        super(Text.literal(""));
        editingIndex = -1;
        this.signPos = signPos;
        final ClientLevel world = Minecraft.getInstance().level;
        for (final BlockModernSign.SignType signType : BlockModernSign.SignType.values()) {
        ...
        if (world != null) {
            ...
            if (entity instanceof BlockModernSign.TileEntityModernSign signEntity) {
            ...
            if (world.getBlockState(signPos).getBlock() instanceof BlockModernSign) {
                length = ((BlockModernSign) world.getBlockState(signPos).getBlock()).length;
            } else {
                length = 0;
            }
        } else {
    ...
    protected void init() {
        ...
        if (!isModernSign && minecraft != null) {
    ...
    public void render(PoseStack matrices, int mouseX, int mouseY, float delta) {
        try {
            ...
            for (int i = 0; i < signIds.length; i++) {
                if (signIds[i] != null) {
                    RenderModernSign.drawSign(matrices, null, null, font, signPos, signIds[i], (width - SIGN_SIZE * length) / 2F + i * SIGN_SIZE, 0, SIGN_SIZE, RenderModernSign.getMaxWidth(signIds, i, false), RenderModernSign.getMaxWidth(signIds, i, true), selectedIds, Direction.UP, 0, (textureId, x, y, size, flipTexture) -> {
            ...
            if (editingIndex >= 0) {
                ...
                loopSigns((index, x, y, isBig) -> {
                    ...
                    if (sign != null) {
                        ...
                        RenderModernSign.drawSign(matrices, null, null, font, signPos, signId, (isBig ? xOffsetBig : xOffsetSmall) + x + (moveRight ? SIGN_BUTTON_SIZE * 2 : 0), BUTTON_Y_START + y, SIGN_BUTTON_SIZE, 2, 2, selectedIds, Direction.UP, 0, (textureId, x1, y1, size, flipTexture) -> blit(matrices, (int) x1, (int) y1, 0, 0, (int) size, (int) size, (int) (flipTexture ? -size : size), (int) size));
                    }
                }, false);
    ...
    @Override
    public void onClose() {
        IVRPacketTrainDataGuiClient.sendModernSignIdsC2S(signPos, selectedIds, signIds, luminance);
        super.onClose();
    }
    ...
    private void setNewSignId(String newSignId) {
        if (editingIndex >= 0 && editingIndex < signIds.length) {
            ...
            final boolean isExitLetter = newSignId != null && (newSignId.equals(BlockModernSign.SignType.EXIT_LETTER.toString()) || newSignId.equals(BlockModernSign.SignType.EXIT_LETTER_FLIPPED.toString()));
            final boolean isPlatform = newSignId != null && (newSignId.equals(BlockModernSign.SignType.PLATFORM.toString()) || newSignId.equals(BlockModernSign.SignType.PLATFORM_FLIPPED.toString()));
            final boolean isLine = newSignId != null && (newSignId.equals(BlockModernSign.SignType.LINE.toString()) || newSignId.equals(BlockModernSign.SignType.LINE_FLIPPED.toString()));
            final boolean isStation = newSignId != null && (newSignId.equals(BlockModernSign.SignType.STATION.toString()) || newSignId.equals(BlockModernSign.SignType.STATION_FLIPPED.toString()));
    ...
    private void setLuminance(boolean luminance) {
        if (isModernSign) this.luminance = luminance;
